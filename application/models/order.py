...
from application.errors import UnexpectedStatus
...
class Order(MongoModels):
    ...
    def is_over_sla(self) -> bool:
        if self.is_terminal():
            ...
            # 10 mins for users to fill out payment method, and for auth to be terminal
            return now_in_epoch_sec() - self.created_at > 60 * 1
        elif self.is_booked():
            # 24 hrs
            return now_in_epoch_sec() - self.created_at > 60 * 60 * 24

    def next_recovery_schedule(self) -> int:
        if self.is_created():
            return 30
        elif self.is_booked():
            if now_in_epoch_sec() - self.created_at > 60 * 5:
                # first 5 mins
                return 5
            elif now_in_epoch_sec() - self.created_at > 60 * 20:
                # 5 to 20 mins
                return 60
            elif now_in_epoch_sec() - self.created_at > 60 * 60:
                # less than one hour
                return 60 * 5
            elif now_in_epoch_sec() - self.created_at > 60 * 60 * 10:
                # less than one hour, 15 mins
                return 60 * 15
            elif now_in_epoch_sec() - self.created_at > 60 * 60 * 24:
                return 60 * 30
    ...
    def on_auth_success(self) -> None:
        self.status = ORDER_STATUS_BOOKED
        self.incoming_invoice.status = INVOICE_STATUS_PENDING

    def on_capture_success(self) -> None:
        self.status = ORDER_STATUS_FULFILLED
        self.incoming_invoice = INVOICE_STATUS_COMPLETED
